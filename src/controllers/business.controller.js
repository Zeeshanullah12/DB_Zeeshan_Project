...
exports.uploadBusinessImage = async (req, res, next) => {
    try {
        let file = req.file;
        if (!file) throw new customError.NotFoundError("No file uploaded or invalid file type!")

        res.status(200).json({ message: 'File uploaded successfully!', filePath: `../assets/images/${req.file.filename}` });
    } catch (err) {
        next(err)
    }
}
...
exports.createBusiness = async (req, res, next) => {
    try {
        ...
        if (!addBusiness) throw new customError.NotFoundError("Business creation fail!")
...
exports.login = async (req, res, next) => {
    try {
        ...
        const business = await BusinessModel.findOne({ email: payload.email, isDeleted: false });
        if (!business) throw new customError.NotFoundError("Email does not exsist!");
        if (business?.isApproved && business?.isApproved == true) throw new customError.UnauthorizedError('Access denied: approval pending!');

        isMatch = await encryptionHelper.compareHash(payload.password, business.password);
        if (!isMatch) throw new customError.UnauthorizedError("Invalid credentials!");
...
exports.updateBusiness = async (req, res, next) => {
    try {
        ...
        const business = await BusinessModel.findOne({ _id: _id, email: payload.email, isDeleted: false });
        if (!business) throw new customError.NotFoundError("Buseiness does not exsist!")
        
        payload.updatedAt = new Date()
        const addBusiness = await crudService.updateById(BusinessModel, _id, payload)
